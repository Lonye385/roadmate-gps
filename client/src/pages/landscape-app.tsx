console.log('[ROADMATE LANDSCAPE] 📦 HORIZONTAL GPS MODULE LOADING...');
...
export default function LandscapeApp() {
  console.log('[ROADMATE LANDSCAPE] 🚀 HORIZONTAL GPS RENDERING!');
  
  // Detect device type and redirect if needed
  // Telemóveis (portrait ou <768px) → /app (vertical)
  // Tablets (landscape e ≥768px) → /app-landscape (horizontal)
  useEffect(() => {
    const checkDeviceOrientation = () => {
      const isPortrait = window.innerHeight > window.innerWidth;
      const isSmallScreen = window.innerWidth < 768; // Less than tablet size
      
      // If portrait mode OR small screen (telemóvel), redirect to /app
      if (isPortrait || isSmallScreen) {
        console.log('[LANDSCAPE] 📱 Redirecting to /app (telemóvel/portrait detected)');
  ...
  useEffect(() => {
    ...
    console.log('[LANDSCAPE] 🗺️ Initializing MapLibre GL...');
    ...
    mapInstance.on('load', () => {
      console.log('[LANDSCAPE] ✅ Map loaded!');
      setMapLoaded(true);
    });
    ...
    const calculateRoute = async () => {
      try {
        ...
        if (data.features && data.features[0]) {
          ...
          if (route.properties.legs && route.properties.legs.length > 0) {
            ...
            route.properties.legs.forEach((leg: any, legIndex: number) => {
              ...
              if (!legGeometry || !Array.isArray(legGeometry)) {
                console.warn('[TURN-BY-TURN] ⚠️ Missing geometry for leg', legIndex);
                return;
              }
              ...
              leg.steps.forEach((step: any) => {
                ...
                if (!stepCoords || stepCoords.length === 0) {
                  console.warn('[TURN-BY-TURN] ⚠️ Skipping step with empty coordinates:', instruction);
                  return;
                }
            ...
            console.log('[TURN-BY-TURN] ✅ Parsed', steps.length, 'navigation steps from', route.properties.legs.length, 'legs');
          ...
          console.log('[ROUTE DEBUG] 📍 Flattened polyline:', flattenedCoords.length, 'coordinates');
          ...
          if (map.current!.getSource('route')) {
            ...
            console.log('[ROUTE DEBUG] ✅ Route layer added successfully!');
          }
  ...
  useEffect(() => {
    ...
    if (distanceToStep < 50 && currentStepIndex < navigationSteps.length - 1) {
      console.log('[TURN-BY-TURN] ✅ Step completed! Advancing to next instruction...');
    ...
    if (routeCoords.length > 0) {
      ...
      if (minDistanceToRoute > 100 && !isOffRoute) {
        console.log('[TURN-BY-TURN] ⚠️ OFF ROUTE! Distance:', minDistanceToRoute, 'm');
        setIsOffRoute(true);
  ...
  useEffect(() => {
    ...
    console.log('[REROUTING] 🔄 User went off-route! Recalculating in 3 seconds...');
        ...
        if (data.features && data.features[0]) {
          ...
          if (route.properties.legs && route.properties.legs.length > 0) {
            ...
            route.properties.legs.forEach((leg: any, legIndex: number) => {
              ...
              const legGeometry = route.geometry.coordinates[legIndex];
              if (!legGeometry || !Array.isArray(legGeometry)) {
                console.warn('[REROUTING] ⚠️ Missing geometry for leg', legIndex);
                return;
              }
              ...
              leg.steps.forEach((step: any) => {
                ...
                // Skip step if coordinates are invalid
                if (!stepCoords || stepCoords.length === 0) {
                  console.warn('[REROUTING] ⚠️ Skipping step with empty coordinates:', instruction);
                  return;
                }
            ...
            console.log('[REROUTING] ✅ Parsed', steps.length, 'navigation steps from', route.properties.legs.length, 'legs');
          ...
          routePolyline.current = flattenedCoords;
          console.log('[REROUTING] 📍 Flattened polyline:', flattenedCoords.length, 'coordinates');
          ...
          setIsOffRoute(false);
          console.log('[REROUTING] ✅ New route calculated!');
        }
      } catch (error) {
  ...
  useEffect(() => {
    ...
    radars.forEach(radar => {
      ...
      if (tierChanged || distanceChanged || distanceIncreased) {
        ...
        if (existingTimer) {
          ...
          console.log('[BEEP] 🔇 Cleared old timer for radar:', radarId);
        }
        ...
        if (tierChanged) {
          console.log('[BEEP] 🔊 Radar', radarId, 'tier changed to:', beepInterval, 'ms (distance:', distance.toFixed(0), 'm)');
        } else if (distanceIncreased) {
          console.log('[BEEP] 🔊 Radar', radarId, 'distance increased, slowing beep (distance:', distance.toFixed(0), 'm)');
        }
      }
    });
    ...
    beepTimersRef.current.forEach((timer, radarId) => {
      if (!currentRadarIds.has(radarId)) {
        ...
        console.log('[BEEP] 🔇 Stopped beeping for radar:', radarId);
      }
    });
    ...
    setActiveRadarAlerts(prevAlerts => {
      ...
      radars.forEach(radar => {
        ...
        if (distance < 150 && !isTracked && !wasAlerted) {
          console.log('[RADAR ALERT] 🚨 Adding radar to active alerts! Distance:', distance.toFixed(0), 'm, ID:', radarId);
        ...
        // CLOSE ALERT when passed the radar (> 250m for hysteresis)
          ...
          console.log('[RADAR ALERT] ✅ Radar passed (>250m), removing from alerts, ID:', radarId);
          ...
          const cooldownTimer = setTimeout(() => {
            ...
            console.log('[RADAR ALERT] 🔄 Cooldown cleared for radar:', radarId);
          }, 30000);
    ...
    >
          ...
          >
            ...
            <button
              onClick={() => {
                ...
                console.log('[LANDSCAPE] ✅ Navigation started!');
              }}
      ...
      <RadarSettingsModal 
        ...
        onSettingsChange={(newSettings) => {
          console.log('[RADAR SETTINGS] 🔧 Settings updated:', newSettings);
      ...
      <style>{`
        /* RESPONSIVE SIDEBAR - Adapta para tablets e telemóveis */
        ...
        /* Tablets pequenos e telemóveis grandes (landscape) */
        ...
        /* Tablets pequenos e telemóveis grandes */
        @media (max-width: 767px) and (min-width: 568px) {
          .eta-time-responsive {
            font-size: 24px;
          }
